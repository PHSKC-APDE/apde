...
#### FUNCTION ####
create_db_connection <- function(server = c("phclaims", "hhsaw", "inthealth", "APDEStore"), 
                                 prod = T, 
                                 interactive = F) {
  ...
  if (server == "phclaims") {
    ...
    conn <- DBI::dbConnect(odbc::odbc(),
                           ...
                           pwd = keyring::key_get("hhsaw", keyring::key_list("hhsaw_dev")[["username"]]),
                           Encrypt = "yes",
